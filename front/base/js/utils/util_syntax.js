...
单一数组去重：
    ...
    方法二：利用splice直接在原数组进行操作，双层循环，外层循环元素，内层循环时比较值，值相同时，则删去这个值
    ...
    方法六：利用ES6的Set数据结构，它类似于数组，其成员的值都是唯一的。利用Array.from将Set结构转换成数组
合并数组去重：
    方法一：concat()方法
 ...
 */
 ...
 function checkType (obj) {
    ...
    // Function,undefined, Null, Date, RegExp, Error
    let type = [];
    let number = 10, 
        string = 'string', 
        boolean = true, 
        ...
        nullz = null, 
        date = new Date(), 
        regexp = new RegExp('abc{2}'), 
        symbol = Symbol();
        error = new Error('this is an error!');
    let arr = [ number, string, boolean, object, array, fn, undef, nullz, date, regexp, symbol, error];
    ...
    // method two:  Object.prototype.toString.call
    arr.forEach((item) => {
