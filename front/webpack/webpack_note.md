...
```shell
    ...
    # 全局安装是为了执行 webpack 任务，
    # 本地安装是为了使用 webpack 提供的 api
```
...
```shell
    ...
    # --port 8888:设置端口、--contentBase ./dist:文件目录、--inline:自动刷新
    npx webpack-dev-server --port 8888 --contentBase ./dist --inline
...
```text
    ...
    由 HotModulePlugin 来完成，提供了相关 API 以供开发者针对自身场景进行处理，
...
### 如何优化 Webpack 的构建速度？(这个问题就像能不能说一说「从URL输入到页面显示发生了什么」一样）
...
```text
    ...
    生成：以新的 AST 为基础生成代码
    想了解如何一步一步实现一个编译器的同学可以移步 Babel 官网曾经推荐的开源项目the-super-tiny-compiler
```
