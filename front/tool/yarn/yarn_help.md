...
`--cwd <cwd>`                         | working directory to use (default: /Users/berb00/Desktop/Github00/Log)
...
`--modules-folder <path>`             | rather than installing modules intothe node_modules folder relative to the cwd, output them here
`--mutex <type>[:specifier]`          | use a mutex to ensure only one yarninstance is executing
...
`-s, --silent`                        | skip Yarn console logs, other typesof logs (script output) will be printed
...
`--verbose`                           | output verbose messages on internaloperations
